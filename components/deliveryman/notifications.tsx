...
export default function DeliverymanNotifications() {
	...
	const { execute: executeGetNotifications, loading: notificationsLoading } =
		useApiCall<any>();
	const { execute: executeAcceptDelivery } = useApiCallWithSuccess(
		'Livraison acceptée avec succès !'
	);
	const { execute: executeRejectDelivery } =
		useApiCallWithSuccess('Livraison refusée');
		...
		try {
			...
			try {
				const notificationsResponse =
					await livreurService.getMyLivraisons();
				console.log(
					'Notifications API récupérées:',
					notificationsResponse
				);

				// Transformer les livraisons en notifications
				const apiNotifications: Notification[] = (
					notificationsResponse.data?.livraisons ||
				...
				console.warn(
					'Erreur API notifications, utilisation des données de fallback:',
					apiError
				);
			}
			...
			console.warn(
				'⚠️ Aucune donnée de livraison disponible pour générer des notifications'
			);
			setNotifications([]);
		} catch (error) {
			console.error(
			...
			if (typeof window !== 'undefined') {
				alert(
					'⚠️ Erreur de connexion: Impossible de charger les notifications. Veuillez vérifier votre connexion internet.'
				);
			}
		}
	};
	...
	const handleAccept = async (id: string) => {
		try {
			...
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_API_URL}/annonces/${id}/livraisons`,
				{
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
				...
				{loading ? (
					...
					<>
						<p className='text-center text-lg mb-8'>
							{t('deliveryman.notificationsDescription')}
						</p>
