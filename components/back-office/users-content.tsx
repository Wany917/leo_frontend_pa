...
export function UsersContent() {
		...
		const fetchUsers = async () => {
			try {
				const response = await fetch(
					`${process.env.NEXT_PUBLIC_API_URL}/utilisateurs/all`
				);
	...
	const deliveryManData = filterUsers(
		...
		justificatives:
			user.justificationPieces?.filter(
				(piece: { accountType: string }) =>
	...
	const serviceProvidersData = filterUsers(
		...
		phone: user.phoneNumber,
		status:
			user.state === 'open' ? t('admin.accepted') : t('admin.rejected'),
		statusColor:
			user.state === 'open'
				? 'bg-[#8CD790] text-white'
				: 'bg-[#E57373] text-white',
	...
	const handleToggleStatus = async (
		...
		const isCurrentlyActive =
			currentStatus === t('admin.active') ||
			currentStatus === t('admin.accepted');
		...
		setConfirmDialog({
			...
			onConfirm: async () => {
				try {
					const response = await fetch(
						`${process.env.NEXT_PUBLIC_API_URL}/admins/toggle-user-status/${userId}`,
						{
							method: 'PUT',
							headers: {
	...
	const handleDelete = (userId: number) => {
		setConfirmDialog({
			...
			onConfirm: () => {
				...
				fetch(
					`${process.env.NEXT_PUBLIC_API_URL}/admins/close-user/${userId}`,
					{
						method: 'PUT',
						headers: {
							'Content-Type': 'application/json',
		...
		<div className='bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-8 text-center'>
			<div className='text-gray-500'>
				...
				<p className='text-sm mt-1'>
					{t('admin.noUsersOfType')} {userType.toLowerCase()}
				</p>
			</div>
		</div>
	);

	// Filter sections based on selected filter and hide empty sections during search
	const shouldShowSection = (sectionType: string, sectionData: any[]) => {
		const matchesFilter =
			selectedFilter === 'all' || selectedFilter === sectionType;

		// If there's a search term, only show sections that have data
	...
	return (
		<div className='space-y-6'>
					...
					<div className='w-full sm:w-64'>
						...
						>
							...
							<SelectContent>
								...
								<SelectItem value='serviceProviders'>
									{t('admin.serviceProviders')}
								</SelectItem>
								<SelectItem value='shopkeepers'>
									{t('admin.shopkeepers')}
								</SelectItem>
			...
			{/* Users Table */}
			<div className='space-y-6'>
				{shouldShowSection('deliveryMan', deliveryManData) && (
					<div>
				...
				{shouldShowSection(
					...
					<div>
						...
						{serviceProvidersData.length > 0 ? (
							...
							<EmptyUserSection
								userType={t('admin.serviceProviders')}
							/>
						)}
					</div>
				)}

				{shouldShowSection('shopkeepers', shopkeepersData) && (
					<div>
						<h2 className='text-xl font-semibold mb-4'>
							{t('admin.shopkeepers')}
						</h2>
						{shopkeepersData.length > 0 ? (
							...
							<EmptyUserSection
								userType={t('admin.shopkeepers')}
							/>
						)}
					</div>
				)}

				{shouldShowSection('clients', usersData) && (
					<div>
						<h2 className='text-xl font-semibold mb-4'>
							{t('admin.clients')}
						</h2>
				...
				{shouldShowSection('administrators', administratorsData) && (
					<div>
						<h2 className='text-xl font-semibold mb-4'>
							{t('admin.administrators')}
						</h2>
						{administratorsData.length > 0 ? (
							<UserTable
								data={administratorsData.map((admin) => ({
									...admin,
									justificatives: [],
								}))}
								showJustificative={false}
							...
							<EmptyUserSection
								userType={t('admin.administrators')}
							/>
						)}
					</div>
				)}
