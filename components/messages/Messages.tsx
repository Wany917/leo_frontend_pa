					...
					if (data.userId) {
						setTypingUsers((prev) => {
							if (prev[data.userId]?.timeout) {
								clearTimeout(
									prev[data.userId].timeout as NodeJS.Timeout
								);
							}
	...
	const formatConversations = (messages: any[], currentUserId: number) => {
		...
		messages.forEach((msg) => {
			...
			const senderLastName =
				msg.senderId === currentUserId
					? ''
					: otherUser?.last_name || otherUser?.lastName || '';
			const senderName =
				msg.senderId === currentUserId
					? 'Vous'
			...
			if (
				...
				new Date(conversation.lastMessageTime || 0)
			) {
	...
	const handleNewMessage = (message: any) => {
		...
		if (existingConvIndex >= 0) {
			setConversations((prevConversations) => {
				...
				if (
					tempId &&
					conversation.messages.some((m) => m.id === tempId)
				) {
