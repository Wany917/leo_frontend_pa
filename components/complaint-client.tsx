...
export default function ComplaintClient() {
	...
	const filteredComplaints = complaints.filter((item) => {
		...
		if (activeTab === 'resolved')
			return ['done', 'rejected'].includes(item.status);
		return true;
	});
	...
	const fetchUserAnnouncements = async (userId: number) => {
		try {
			...
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_API_URL}/annonces/user/${userId}`,
				{
					headers: {
						Authorization: `Bearer ${token}`,
					},
			...
			if (response.ok) {
				...
				const annoncesData = Array.isArray(data)
					? data
					: data.data || data.annonces || [];

				if (annoncesData.length > 0) {
					const formattedAnnouncements = annoncesData.map(
						(item: any) => ({
							id: item.id,
