	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (
				...
				setError(
					'Veuillez remplir tous les champs obligatoires (adresse, titre, prix, nom du colis, taille et poids)'
				);
			...
			// Priorité du colis
			const isPriority =
				packages[currentPackage - 1]?.priorityShipping || false;
			...
			if (startingType === 'box' && startingBox) {
				...
				console.log(
					"Aucun storage box sélectionné ou type d'adresse utilisé"
				);
			}
			...
			if (!response.ok) {
				...
				throw new Error(
					`${t('announcements.errorCreating')}: ${JSON.stringify(
						errorData
					)}`
				);
			}
			...
			<main className='container mx-auto px-4 py-8'>
				<div className='max-w-2xl mx-auto'>
						...
						<div className='bg-white rounded-lg shadow-md p-6'>
							...
							<div className='space-y-6'>
								...
								<div className='flex justify-between pt-4'>
									...
									>
										{t(
											'announcements.continueToAnnouncementDetails'
										)}
									</button>
								</div>
							</div>
					...
					{step === 3 && (
						<div className='bg-white rounded-lg shadow-md p-6'>
							<h2 className='text-xl font-medium text-gray-800 mb-6'>
								{t('announcements.announcementDetails')}
							</h2>
							...
							<div className='space-y-6'>
								...
								<div>
									...
									<input
										...
										className='w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500'
										placeholder={t(
											'announcements.enterAnnouncementTitle'
										)}
										required
									/>
								</div>
							...
							<div className='flex justify-between pt-6'>
								...
								>
									{t(
										'announcements.continueToPackageDetails'
									)}
								</button>
							</div>
						</div>
							...
							<form onSubmit={handleSubmit} className='space-y-6'>
								{packages.map((pkg, index) => (
									...
									>
										...
										<div className='mt-8'>
											<label
												htmlFor={`package-notes-${index}`}
												className='block text-sm font-medium text-gray-700 mb-1'
											>
												{t(
													'announcements.packageNotes'
												)}{' '}
												({t('common.optional')})
											</label>
											<textarea
												...
												rows={3}
												placeholder={t(
													'announcements.enterPackageNotes'
												)}
											/>
										</div>
